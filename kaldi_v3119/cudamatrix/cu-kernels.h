...
template<typename Real> inline void cuda_apply_power(dim3 Gr, dim3 Bl, Real *mat, Real pow, MatrixDim d) { KALDI_ERR << __func__ << " Not implemented!"; }
...
template<> inline void cuda_apply_power<float>(dim3 Gr, dim3 Bl, float *mat, float pow, MatrixDim d) { cudaF_apply_power(Gr,Bl,mat,pow,d); }
template<> inline void cuda_apply_exp<float>(dim3 Gr, dim3 Bl, float *mat, MatrixDim d) { cudaF_apply_exp(Gr,Bl,mat,d); }template<> inline void cuda_apply_log<float>(dim3 Gr, dim3 Bl, float *mat, MatrixDim d) { cudaF_apply_log(Gr,Bl,mat,d); }
...
template<> inline void cuda_apply_power<double>(dim3 Gr, dim3 Bl, double *mat, double pow, MatrixDim d) { cudaD_apply_power(Gr,Bl,mat,pow,d); }
