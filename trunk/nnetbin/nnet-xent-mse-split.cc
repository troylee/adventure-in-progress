...
int main(int argc, char* argv[]){
  try{
    ...
    const char *usage =
        "Split the double objective Nnet to shared one and two separate ones.\n"
        "Usage: nnet-xent-mse-split <ori-nnet-filename> <out-shared-filename>"
        "<out-xent-filename> <out-mse-filename>\n"
        "e.g. :\n"
        "nnet-xent-mse-split ori.nnet shared.nnet xent.nnet mse.nnet\n";
    ...
    std::string ori_nnet_filename = po.GetArg(1),
        out_shared_filename = po.GetArg(2),
        out_xent_filename = po.GetArg(3),
        out_mse_filename = po.GetArg(4);
    ...
    /*
     * Write out the shared lower layers.
     */
    {
      Output ko(out_shared_filename, binary);
      for(int32 i=0; i < nnet.LayerCount()-1; ++i){
        (nnet.Layer(i))->Write(ko.Stream(), binary);
      }

      KALDI_LOG << "Write shared nnet layers to " << out_shared_filename << ".\n";
    }

    /*
     * Split the last layer
     */
    BiasedLinearity &bl= dynamic_cast<BiasedLinearity&>(*(nnet.Layer(nnet.LayerCount()-1)));
    ...
    if (xent_dim > 0){
      ...
      if(!binary) (ko.Stream())<< "\n";
      (SubMatrix<BaseFloat>(weight, 0, xent_dim, 0, dim_in)).Write(ko.Stream(), binary);
    ...
    if(dim_mse > 0){
      ...
      WriteBasicType(ko.Stream(), binary, dim_in);
      if(!binary) (ko.Stream())<< "\n";
      (SubMatrix<BaseFloat>(weight, xent_dim, dim_mse, 0, dim_in)).Write(ko.Stream(), binary);
      (SubVector<BaseFloat>(bias, xent_dim, dim_mse)).Write(ko.Stream(), binary);
