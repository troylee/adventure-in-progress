/*
 * vtsbin/dbnvts2-forward-interpolate.cc
 *
 ...
 *
 *  We interpolate the new decision boundary with the original one.
 *
 */
  ...
  try {
    const char *usage =
        "Estimate the new decision boundaries and perform forward pass through Neural Network.\n"
            "Usage:  dbnvts2-forward-interpolate [options] <pos-model-in> <neg-model-in> <bl-layer>"
            " <back-nnet-in> <global-cmvn> <pos2neg-prior-in> <var-scale-in> <llr-scale-in> <feature-rspecifier>"
            " <noise-model-rspecifier> <feature-wspecifier>\n"
            "e.g.: \n"
            " dbnvts2-forward pos.mdl neg.mdl bl_layer nnet.back ark:global_cmvn.ark pos2neg.stats var_scale.stats llr_scale.stats"
            "ark:features.ark ark:noise_params.ark ark:mlpoutput.ark\n";

    ParseOptions po(usage);

    BaseFloat estimate_weight = 1.0;
    po.Register("estimate-weight", &estimate_weight,
                "The weight for the new estimation, should be in range [0, 1]");
    ...
    std::string pos_model_filename = po.GetArg(1), neg_model_filename = po
        .GetArg(2), bl_layer_filename = po.GetArg(3), nnet_filename = po.GetArg(
        4), global_cmvn_rspecifier = po.GetArg(5), pos2neg_prior_filename = po
        .GetArg(6), var_scale_filename = po.GetArg(7), llr_scale_filename = po
        .GetArg(8), feature_rspecifier = po.GetArg(9), noise_params_rspecifier =
        po.GetArg(10), feature_wspecifier = po.GetArg(11);
    ...
    RandomAccessDoubleMatrixReader cmvn_reader(global_cmvn_rspecifier);
    ...
    for (; !feature_reader.Done(); feature_reader.Next()) {
      ...
      if (shared_var) {
        ComputeGaussianLogLikelihoodRatio_Interpolate(feat, pos_noise_am, neg_noise_am,
                                          pos2neg_log_prior_ratio, llr_scale,
                                          linearity, bias, estimate_weight,
                                          mat);
      } else {
        KALDI_ERR<< "Interpolation only allows shared covariance Gaussians!";
      }
