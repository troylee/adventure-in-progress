 ...
 *
 *  The positive and negative Gaussians are always constrained to
 *  share the covariance, thus the boundary is linear.
 *
 */
  ...
  try {
    const char *usage =
        "Estimate the new decision boundaries and perform forward pass through Neural Network.\n"
            "Usage:  dbnvts-forward [options] <biasedlinearity-layer-in> <pos-model-in> <neg-model-in>"
            " <back-nnet-in> <pos2neg-prior-in> <var-scale-in> <feature-rspecifier>"
            " <noise-model-rspecifier> <feature-wspecifier>\n"
            "e.g.: \n"
            " dbnvts-forward bl_layer pos.mdl neg.mdl nnet.back ark:features.ark ark:noise_params.ark ark:mlpoutput.ark\n";
    ...
    for (; !feature_reader.Done(); feature_reader.Next()) {
      ...
      CompensateMultiFrameGmm(mu_h, mu_z, var_z, compensate_var, num_cepstral, num_fbank,
                              dct_mat, inv_dct_mat, num_frames, pos_noise_am);

      neg_noise_am.CopyFromAmDiagGmm(neg_am_gmm);
      CompensateMultiFrameGmm(mu_h, mu_z, var_z, compensate_var, num_cepstral, num_fbank,
                              dct_mat, inv_dct_mat, num_frames, neg_noise_am);
      ...
      ComputeGaussianBoundary(pos_noise_am, neg_noise_am,
                              pos2neg_log_prior_ratio, var_scale, vts_linearity,
                              vts_bias);
