...
int main(int argc, char *argv[]) {
  try {
    ...
    po.Register("feat-dim", &feat_dim, "Number of first N dimensions to output.");
    po.Register("target-pdf", &target_pdf, "Target PDF ID to select the features, starts from 0.");
    ...
    for (; !feats_reader.Done() ; feats_reader.Next()) {
      ...
      for(int32 i=0; i<pdfs.size(); ++i){
        if(pdfs[i]==target_pdf) ++tot_frames;
      }
